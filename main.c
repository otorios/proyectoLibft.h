...
/*   Updated: 2024/10/21 19:12:40 by olalsanc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	printf("Test 1 - valor = %d (antes del espacio):
		%d (no es un carácter imprimible)\n", value, ft_isprint(value));
	...
	//***************************************************
	printf("\nTest de ft_memcpy()\n");
	//Cuando trabajas con cadenas de caracteres en C y utilizas funciones como printf con el formato
		%s, el formato
		%s está diseñado específicamente para trabajar con cadenas de caracteres (arrays de char) que están terminadas en un carácter nulo ('\0').
	//La función printf reconoce automáticamente el carácter nulo para saber dónde termina la cadena y por lo tanto,
		no necesitas un bucle explícito para imprimir cada carácter.
	char	dest[6] = "Hola";// que teng el espacio para la palabra que queremos meter,
			ene ste caso mundo
	num = 5;
	...
	num_ints = 5 * sizeof(int); // Número de bytes a copiar (5 enteros);
	...
	//Dado que cada entero puede ser de 4 bytes en la mayoría de las plataformas,
		estás copiando solo 5 bytes,
		lo cual no es suficiente para copiar todos los enteros.
...
int	main(void)
{
	...
	printf("\nTest de ft_strrchr()\n");
	y = 'l';
	result = ft_strrchr(strArrayR, y);
	if (result != NULL)
	{
		printf("Buscamos \"%c\" en la palabra \"%s\", y está en la posición
	...
	if (result != NULL)
	{
		printf("Buscamos el carácter nulo en la palabra \"%s\",
			y está en la posición %ld\n", strArrayR5, result - strArrayR5);
	}
	else
	{
		printf("Buscamos el carácter nulo en la palabra \"%s\",
...
printf("Buscamos \"%c\" en el bloque \"%s\", y está en la posición %ld\n",
		x_memchr, strArray_memchr, (char *)ft_memchr(strArray_memchr, x_memchr,
			sizeof(strArray_memchr)) - strArray_memchr);

char	strArray2_memchr[10] = "Hola mundo";
char	x2_memchr = 'z';
printf("Buscamos \"%c\" en el bloque \"%s\", y está en la posición %ld\n",
		x2_memchr, strArray2_memchr, (char *)ft_memchr(strArray2_memchr,
			x2_memchr, sizeof(strArray2_memchr)) - strArray2_memchr);

char	strArray3_memchr[10] = "Hola mundo";
char	x3_memchr = ' ';
printf("Buscamos \"%c\" en el bloque \"%s\", y está en la posición %ld\n",
		x3_memchr, strArray3_memchr, (char *)ft_memchr(strArray3_memchr,
			x3_memchr, sizeof(strArray3_memchr)) - strArray3_memchr);

char	strArray4_memchr[10] = "Hola mundo";
char	x4_memchr = 'H';
printf("Buscamos \"%c\" en el bloque \"%s\", y está en la posición %ld\n",
		x4_memchr, strArray4_memchr, (char *)ft_memchr(strArray4_memchr,
			x4_memchr, sizeof(strArray4_memchr)) - strArray4_memchr);

char	strArray5_memchr[10] = "Hola mundo";
char	x5_memchr = '\0';
printf("Buscamos \"%c\" en el bloque \"%s\", y está en la posición %ld\n",
		x5_memchr, strArray5_memchr, (char *)ft_memchr(strArray5_memchr,
			x5_memchr, sizeof(strArray5_memchr)) - strArray5_memchr);

char	strArray6_memchr[10] = "Hola mundo";
char	x6_memchr = 'd';
printf("Buscamos \"%c\" en el bloque \"%s\", y está en la posición %ld\n",
		x6_memchr, strArray6_memchr, (char *)ft_memchr(strArray6_memchr,
			x6_memchr, sizeof(strArray6_memchr)) - strArray6_memchr);
