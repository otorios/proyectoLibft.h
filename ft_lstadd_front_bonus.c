...
#include "libft.h"      // Para la definición de t_list y las funciones

// Función para añadir un nodo al principio de la lista
void ft_lstadd_front(t_list **lst, t_list *new)
{
    // Verificamos si el puntero a la lista o el nuevo nodo son NULL
    if (lst == NULL || new == NULL)
        return ; // Si es así, no hacemos nada
    
    // Hacemos que el nuevo nodo apunte al primer nodo de la lista
    new->next = *lst; // El nuevo nodo ahora apunta al antiguo primer nodo
    
    // Actualizamos el puntero al primer nodo para que apunte al nuevo nodo
    *lst = new; // Ahora el puntero a la lista apunta al nuevo nodo
}
...
void print_list(t_list *lst)
{
    ...
    { // Mientras haya nodos en la lista
        printf("%d -> ", lst->content); // Imprimimos el valor del nodo actual
